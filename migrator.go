...
func (m Migrator) HasTable(value interface{}) bool {
	var count int64
	m.RunWithValue(value, func(stmt *gorm.Statement) error {
		return m.DB.Raw("SELECT count(*) FROM system.tables WHERE name=?", stmt.Table).Scan(&count).Error
	})
...
func (m Migrator) HasColumn(value interface{}, field string) bool {
	...
	m.RunWithValue(value, func(stmt *gorm.Statement) error {
		...
		return m.DB.Raw("SELECT count(*) FROM system.columns WHERE table = ? AND name=?", stmt.Table, name).Row().Scan(&count)
	})
